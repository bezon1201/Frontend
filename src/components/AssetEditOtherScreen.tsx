...
// Screen: AssetEditOtherScreen
//
// Редактирование/добавление одного актива класса "Other".
// Открывается из AccountAssetsSheet.
//
// API (Assets) — те же эндпоинты:
// - GET    /api/assets/{asset_id}
// - POST   /api/accounts/{account_id}/assets
// - PUT    /api/assets/{asset_id}
// - DELETE /api/assets/{asset_id}
//
// UI такой же, как у Fiat (simple):
// - без Trading parameters,
// - без Sim/Live.
//
// asset_class для этого экрана всегда "Other".
...
interface AssetEditOtherScreenProps {
  isOpen: boolean;
  onBack: () => void;
...
export default function AssetEditOtherScreen({ 
  isOpen, 
  onBack, 
  onSave, 
  onDelete,
  ...
  // TODO API (Assets):
  // В этом экране сохраняются только базовые поля, как и в AssetEditFiatScreen:
  // - name
  // - invested_value
  ...
  //
  // Режимы:
  // - add mode    → POST /api/accounts/{account_id}/assets
  // - edit mode   → PUT  /api/assets/{asset_id}
  //
  // После успешного ответа:
  // - закрыть экран,
  // - обновить список активов в AccountAssetsSheet,
  // - показать toast об успешном сохранении/удалении.
  const handleSave = () => {
    if (amountInput) {
      ...
      >
        ...
        <div className="mb-6 flex items-center justify-between">
          <h1 style={{ fontSize: '24px', fontWeight: 'bold' }} className="text-white mb-1">
            {isNewAsset ? 'Add Other' : `Edit ${name}`}
          </h1>
