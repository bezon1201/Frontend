...
// Component: ConfirmLimitSheet
//
// Назначение:
// - bottom-sheet для подтверждения массовой операции "Open limit orders"
//   по ордерам на OrdersScreen.
//
// Поток:
// - открывается из OrdersScreen при нажатии "Open Limit";
// - при Confirm вызывает onConfirm();
// - на уровне OrdersScreen onConfirm должен вызвать
//   POST /api/campaigns/{campaign_id}/orders/open-limit
//   с нужным набором order_ids.
//
// Учитывать trade_mode (Sim/Live) должен только бэкенд,
// этот компонент показывает только подтверждение.

interface ConfirmLimitSheetProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: () => void;
  pendingCount: number;
...
export default function ConfirmLimitSheet({ 
  isOpen, 
  onClose, 
  onConfirm,
  pendingCount 
}: ConfirmLimitSheetProps) {
        ...
        <div className="h-full flex flex-col p-6">
          ...
          <div className="flex-1 flex flex-col items-center justify-center">
            ...
            <div className="text-[20px] text-gray-600 mb-6">
              You are about to open {pendingCount} limit orders for BNBUSDC.
            </div>
            ...
            <div className="w-full max-w-xs space-y-3 mb-6">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-[20px] font-bold text-black">204$</span>
              </div>
            </div>
